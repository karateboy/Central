...
@Singleton
class Report @Inject()(monitorDB: MonitorDB, monitorTypeOp: MonitorTypeDB, recordOp: RecordDB, query: Query,
                       excelUtility: ExcelUtility, groupDB: GroupDB) extends Controller {
  ...
  def getReport(monitor: String, reportTypeStr: String, startNum: Long, outputTypeStr: String) = Security.Authenticated.async {
    implicit request =>
      val userInfo = request.user
      val reportType = PeriodReport.withName(reportTypeStr)
      val outputType = OutputType.withName(outputTypeStr)
      val mtList = monitorTypeOp.activeMtvList.filter({
        val m = monitorDB.map(monitor)
        m.monitorTypes.contains
      })
  ...
  def monitorMonthlyHourReport(monitor: String, mt: String, startDate: Long, outputTypeStr: String) = Security.Authenticated.async {
    implicit request =>
      val userInfo = request.user
      for (groupOpt <- groupDB.getGroupByIdAsync(userInfo.group)) yield {
        val group = groupOpt.get
